...
Ici le modèle est :  $V_2=aV_1+b$


 
\

Si la régression linéaire est avérée, alors pour un individu $i$ dont on connait $V1_i$, on infère la valeur $V2_i$ par le modèle :    $\hat{V_{2i} = a \times V_{1i} +b$
 
 On dit aussi que $V1$ **explique** $V2$ , ou que le modèle **prédit** $V_2$ à partir de $V_1$ (on nte les valeurs prédites $\hat{V2_i}$)
...
Le $R^2$ est le ratio de la variance de $V_2$ expliquée par $V_1$ (i.e. estimée par le modèle $aV_1+b$ ) et de la variance de $V_2$
...
en considérant que  les deux séries ne sont pas indépendantes et qu'il existe une relation entre les deux (ici, un  linéaire car nous testons le coefficient de coorélation linéaire).
...
Dans le cas d'une liaison statistique linéaire entre 2 variables , on peut calculer l'«intensité» de ce lien : leur  **corrélation**

$cor(x,y) \in [-1;1]$ entre deux variables x et y (e.g. colonnes d'un dataframe) .
...
## Réaliser une régression linéaire avec R
